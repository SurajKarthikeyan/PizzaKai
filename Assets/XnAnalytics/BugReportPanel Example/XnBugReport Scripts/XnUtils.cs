...
#region Bounds Functions
		...
		public static Bounds BoundsUnion( Bounds b0, Bounds b1 ) {
			...
			if ( b0.size  == Vector3.zero && b1.size != Vector3.zero ) { return ( b1 ); } else if
				( b0.size != Vector3.zero && b1.size == Vector3.zero ) { return ( b0 ); } else if
				( b0.size == Vector3.zero && b1.size == Vector3.zero ) { return ( b0 ); }
		...
		//			loc.z = -XnUtils.mainCamera.transform.position.z;
		//			loc = XnUtils.mainCamera.ScreenToWorldPoint(loc);
		//			return(loc);
		//		}
		//	}
		static public Vector3 MouseLoc {
		...
		public static Vector3
			ScreenBoundsCheck( Bounds bnd, BoundsTest test = BoundsTest.center ) {
		...
		public static Vector3 BoundsInBoundsCheck( Bounds bigB, Bounds lilB,
			...
			case BoundsTest.center:
				...
				if ( pos.x  > bigB.max.x ) { off.x = pos.x - bigB.max.x; } else if
					( pos.x < bigB.min.x ) { off.x = pos.x - bigB.min.x; }

				if ( pos.y  > bigB.max.y ) { off.y = pos.y - bigB.max.y; } else if
					( pos.y < bigB.min.y ) { off.y = pos.y - bigB.min.y; }

				if ( pos.z  > bigB.max.z ) { off.z = pos.z - bigB.max.z; } else if
					( pos.z < bigB.min.z ) { off.z = pos.z - bigB.min.z; }
			...
			case BoundsTest.onScreen:
				...
				if ( lilB.max.x  > bigB.max.x ) { off.x = lilB.max.x - bigB.max.x; } else if
					( lilB.min.x < bigB.min.x ) { off.x = lilB.min.x - bigB.min.x; }

				if ( lilB.max.y  > bigB.max.y ) { off.y = lilB.max.y - bigB.max.y; } else if
					( lilB.min.y < bigB.min.y ) { off.y = lilB.min.y - bigB.min.y; }

				if ( lilB.max.z  > bigB.max.z ) { off.z = lilB.max.z - bigB.max.z; } else if
					( lilB.min.z < bigB.min.z ) { off.z = lilB.min.z - bigB.min.z; }
			...
			case BoundsTest.offScreen:
				...
				// if not, find the offset
				if ( lilB.min.x  > bigB.max.x ) { off.x = lilB.min.x - bigB.max.x; } else if
					( lilB.max.x < bigB.min.x ) { off.x = lilB.max.x - bigB.min.x; }

				if ( lilB.min.y  > bigB.max.y ) { off.y = lilB.min.y - bigB.max.y; } else if
					( lilB.max.y < bigB.min.y ) { off.y = lilB.max.y - bigB.min.y; }

				if ( lilB.min.z  > bigB.max.z ) { off.z = lilB.min.z - bigB.max.z; } else if
					( lilB.max.z < bigB.min.z ) { off.z = lilB.max.z - bigB.min.z; }
...
#region Bézier Curves
		...
		// This uses the XnUtils.Lerp function because it needs to allow extrapolation
		static public Vector3 Bezier( float u, List<Vector3> vList, int n0 = 0, int n1 = -1 ) {
			...
			Vector3 res = Lerp( Bezier( u, vList, n0, n1 - 1 ), Bezier( u, vList, n0 + 1, n1 ),
				u );
		...
		static public Vector2 Bezier( float u, List<Vector2> vList, int n0 = 0, int n1 = -1 ) {
			...
			// Recur Right: if vList = [0,1,2,3,4] then vListL = [0,1,2,3]
			// The result is the Lerp of these two shorter Lists
			Vector2 res = Lerp( Bezier( u, vList, n0, n1 - 1 ), Bezier( u, vList, n0 + 1, n1 ),
				u );
		...
		static public Quaternion
			Bezier( float u, List<Quaternion> vList, int n0 = 0, int n1 = -1 ) {
			// handle default n1 value
			if ( n1 == -1 ) n1 = vList.Count - 1;
			...
			// Recur Right: if vList = [0,1,2,3,4] then vListL = [0,1,2,3]
			// The result is the Lerp of these two shorter Lists
			Quaternion res = Quaternion.Slerp( Bezier( u, vList, n0, n1 - 1 ),
				Bezier( u, vList, n0                                    + 1, n1 ), u );
...
#region Color Functions
		...
		public static Color HexToColor( string hexString ) {
			...
			if ( len == 6 || len == 8 ) {
				r = int.Parse( hexString.Substring( offset + 0, 2 ),
					System.Globalization.NumberStyles.HexNumber );

				g = int.Parse( hexString.Substring( offset + 2, 2 ),
					System.Globalization.NumberStyles.HexNumber );

				b = int.Parse( hexString.Substring( offset + 4, 2 ),
					System.Globalization.NumberStyles.HexNumber );

				if ( len == 8 ) {
					a = int.Parse( hexString.Substring( offset + 6, 2 ),
						System.Globalization.NumberStyles.HexNumber );
				}
...
#region String and Logging Functions
		...
		static public string Str( object o ) {
			...
			if ( o.GetType().IsArray ) { s = KnownArrayToTabbedString( o ); } else {
				s = o.ToString();

				if ( o is Vector3 && SPLIT_VECTOR_COMPONENTS_ON_LOG ) {
					Vector3 v3 = (Vector3)o;
					s = v3.x + "\t" + v3.y + "\t" + v3.z +
					    "\t";// Inserts a double-tab between Vector3s
				}
		...
		static public string KnownArrayToTabbedString( object o ) {
			...
			foreach ( object obj in (Array)o ) {
				if ( ndx == 0 ) { str += Str( obj ); } else { str += "\t" + Str( obj ); }
		...
		*/
		...
		static public string ArrayToTabbedString( params object[] ss ) {
			...
			//string str = (string) ss[0];
			string str = Str( ss[0] );//ss[0].ToString();
			for ( int i = 1; i < ss.Length; i++ ) { str += "\t" + Str( ss[i] ); }
		...
		static public void SetLogHeader( int ndx, params object[] ss ) {
			MakeUtilsGameObject();
			while ( ndx >= LOG_HEADERS.Count ) { LOG_HEADERS.Add( "" ); }

			LOG_HEADERS[ndx] = ArrayToTabbedString( ss );
		}

		static public string LOG_HEADER {
			get { return ( _LOG_HEADER ); }
			set {
				if ( value != _LOG_HEADER ) { _LOG_HEADER = value; }
			}
		}
		...
		*/
		...
		static public float[] ParseStringListToFloatArray( string eAtt ) {
			...
			for ( int i = 0; i < sArr.Length; i++ ) { fArr[i] = float.Parse( sArr[i] ); }
		...
		static public List<char> MostCommonChars( string s ) {
			...
			foreach ( char cW in s ) {
				for ( int i = 0; i < letters.Length; i++ ) {
					if ( cW == letters[i] ) { letterCounts[i]++; }
				}
			}
...
#region Mathematical Functions
		...
		static public string ToRoman( int number ) {
			...
			if ( ( number < 0 ) || ( number > 3999 ) ) {
				Debug.LogError(
					"Utils.ToRoman: input of out range [1..3999]\t" + number.ToString() );
		...
		static public Vector2 Vector3to2( Vector3 v3, bool useXZ = false ) {
			if ( !useXZ ) { return ( new Vector2( v3.x, v3.y ) ); } else {
...
#region Fuzzy Equality Functions
		...
		static public bool Eq( Vector3 v0, Vector3 v1, float toWithin = 0.1f,
		                       bool checkDimensionsSeparately = false ) {
			return Eq3( v0, v1, toWithin, checkDimensionsSeparately );
		}

		static public bool Eq3( Vector3 v0, Vector3 v1, float toWithin = 0.1f,
		                        bool checkDimensionsSeparately = false ) {
			Vector3 v01 = v1 - v0;
			if ( !checkDimensionsSeparately ) { return ( v01.magnitude <= toWithin ); } else {
				if ( v01.x > -toWithin && v01.x < toWithin ) {
					if ( v01.y > -toWithin && v01.y < toWithin ) {
						if ( v01.z > -toWithin && v01.z < toWithin ) { return ( true ); }
					}
				}
			}
		...
		static public bool Eq( Vector2 v0, Vector2 v1, float toWithin = 0.1f,
		                       bool checkDimensionsSeparately = false ) {
			Vector2 v01 = v1 - v0;
			if ( !checkDimensionsSeparately ) { return ( v01.magnitude <= toWithin ); } else {
				if ( v01.x > -toWithin && v01.x < toWithin ) {
					if ( v01.y > -toWithin && v01.y < toWithin ) { return ( true ); }
				}
			}
...
#region Mesh Functions
		...
		public static void MeshRemoveDuplicateVertices( Mesh m, bool testUV = false,
			...
			for ( int i = 0; i < m.vertexCount; i++ ) {
				if ( vertsToRemove.IndexOf( i ) != -1 )
					continue;// Don't check if we're already removing this vert
				...
				for ( int k = 0; k < triangles.Count; k++ ) {
					...
					if ( identicalVerts.IndexOf( triangles[k] ) > -1 ) { triangles[k] = i; }
				}
				/*
		...
		public struct TriInt3 {
			...
			public static bool operator ==( TriInt3 e1, TriInt3 e2 ) {
				...
				if ( e1.x == e2.x || e1.x == e2.y || e1.x == e2.z ) {
					if ( e1.y == e2.x || e1.y == e2.y || e1.y == e2.z ) {
						if ( e1.z == e2.x || e1.z == e2.y || e1.z == e2.z ) { return ( true ); }
					}
				}
			...
			public static bool operator !=( TriInt3 e1, TriInt3 e2 ) {
				...
				if ( e1.x == e2.x || e1.x == e2.y || e1.x == e2.z ) {
					if ( e1.y == e2.x || e1.y == e2.y || e1.y == e2.z ) {
						if ( e1.z == e2.x || e1.z == e2.y || e1.z == e2.z ) { return ( false ); }
					}
				}
			...
			public override bool Equals( object o ) {
				...
				if ( ti.x != x || ti.y != y || ti.z != z ) { return false; }
		...
		public static void MeshRemoveDuplicateTriangles( Mesh m ) {
			...
			for ( int i = 0; i < m.triangles.Length; i += 3 ) {
				triInts.Add(
					new TriInt3( m.triangles[i], m.triangles[i + 1], m.triangles[i + 2] ) );
			}
...
#region Easing Class
	...
	public class Easing {
		...
		static public float Ease( float u, params string[] curveParams ) {
			...
			if ( cache == null ) { cache = new Dictionary<string, EasingCachedCurve>(); }
		...
		static private void EaseParse( string curveIn ) {
			...
			foreach ( string curve in curves ) {
				...
				if ( curveA.Length == 1 || curveA[1] == "" ) { ecc.mods.Add( float.NaN ); } else {
					float parseRes;
					if ( float.TryParse( curveA[1], out parseRes ) ) {
						ecc.mods.Add( parseRes );
					} else { ecc.mods.Add( float.NaN ); }
				}
			}
		...
		static private float EaseP( float u, string curve, float mod ) {
			...
			case "InOut":
				if ( float.IsNaN( mod ) ) mod = 2;
				if ( u <= 0.5f ) { u2 = 0.5f * Mathf.Pow( u * 2, mod ); } else {
...
#region HexGrid class
	...
	public class XnHexGrid {
		...
		public Vector2 Pos( float col, float row, bool addOddY = true ) {
			...
			float oddY =
				addOddY
					? Mathf.Abs( ( ( col + 1 ) % 2f ) - 1 )
					: 0;// if addOddY == false, oddY = 0
