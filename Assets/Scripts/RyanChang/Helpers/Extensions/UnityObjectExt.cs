...
public static class UnityObjectExt
{
    ...
    /// <returns>True if the singleton was successfully instantiated, false
    /// otherwise. If this returns false, you should immediately stop code
    /// execution of that MonoBehavior.</returns>
    public static bool InstantiateSingleton<T>(this T self, ref T singleton,
        bool dontDestroyOnLoad = true) where T : Component
    {
        if (singleton)
        {
            ...
            Debug.Log($"Multiple instances of {typeof(T)}. Destroying {self}.");
